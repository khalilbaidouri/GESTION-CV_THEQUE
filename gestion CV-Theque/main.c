...
int main() {
    ...
    dernier_id_cv = lire_dernier_id("cvs.txt");  // Nouvelle initialisation
    ...
    Skill* skills = NULL;
    Langue* langues = NULL;
    CV* cvs = NULL;  // Nouvelle liste pour les CVs

    char choix;
    int id_utilisateur_courant = 0; // Stocker l'ID de l'utilisateur connecté
    ...
    do {
        ...
        switch (choix) {
            ...
            case '8': // Gestion des CVs  // Nouveau cas
                if (id_utilisateur_courant == 0) {
                    printf("Veuillez vous connecter d'abord\n");
                } else {
                    gestion_cvs(&cvs, id_utilisateur_courant);
                }
                break;
    ...
    liberer_cvs(cvs);  // Nouvelle libération
