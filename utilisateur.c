        ...
        switch (choix) {
            ...
            case '5': {
                    ...
                    if (result > 0) {
                        ...
                        printf("Échec de la mise a jour de l'utilisateur.\n");
                    }
...
bool est_admin(int id_utilisateur) {
    ...
    while (fgets(ligne, sizeof(ligne), fichier)) {
        // Decouper la ligne en ses 8 elements (ajout du telephone)
...
int mettre_a_jour_utilisateur(int id_utilisateur, const char* nom, const char* prenom,
    ...
    // Construire la nouvelle ligne complète à écrire (avec rôle conservé)
    ...
    // Appeler la fonction générique pour mettre à jour la ligne dans le fichier
