...
void liberer_educations(Education* edu) {
    ...
    // Dans l'ancienne version avec listes, elle libérait la mémoire
    // Maintenant elle ne fait rien car on travaille directement avec fichiers
    while (edu != NULL) {
        Education* temp = edu;
        edu = edu->suivant;
        free(temp);
