...
void ajouter_et_sauvegarder_loisir(Loisir** tete, const char* fichier, int id_utilisateur, char* loisir) {
    ...
    // Trouver le dernier id dans le fichier
    int max_id = 0;
    ...
    do {
        ...
        switch (choix) {
            ...
            case '4': {  // Nouveau cas pour la modification
                ...
                if (result > 0) {
                    printf("Loisir mis � jour avec succ�s!\n");
                } else {
                    printf("�chec de la mise � jour du loisir.\n");
                }
...
int mettre_a_jour_loisir(int id_utilisateur, int id_loisir, const char* nouveau_nom) {
    ...
    if (res > 0) {
        printf("Succ�s : loisir ID %d mis � jour\n", id_loisir);
    } else {
        printf("Aucun loisir correspondant trouv�\n");
    }
